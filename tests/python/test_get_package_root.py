...
def test_build_parser():
    ...
    assert expected == ["root-parameter", "from-git"]


def test_checkout_git_repo(mocker):
    """Test get_package_info.checkout_git_repo()."""
    mock_clone = mocker.patch.object(get_package_root.Repo, "clone_from")

    mock_url = mocker.MagicMock(spec=str)
    mock_path = mocker.MagicMock(spec=pathlib.Path)

    get_package_root.checkout_git_repo(mock_url, mock_path)
...
@pytest.mark.parametrize(
    "git_url",
    ("", "https://github.com/captainhammy/houdini-core-tools.git"),
)
def test_main(monkeypatch, tmp_path, mocker, shared_datadir, git_url):
    ...
    if git_url:
        mock_checkout.assert_called_with(git_url, expected_path)
